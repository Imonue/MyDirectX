...
{
	file = L"../../_Models/" + file + L".mesh"; // 파일 경로를 가져온다
	...
	UINT count = 0;
	count = r->UInt(); // 반복하는 횟수
	for (UINT i = 0; i < count; i++) // 본의 정보를 읽어와서 저장하는 반복문
	{
	...
	count = r->UInt(); // 반복하는 횟수
	for (UINT i = 0; i < count; i++) // 메쉬의 정보를 읽어와서 저장하는 반복문
	{
		...
		//VertexData
		{
			...
			vertices.assign(count, Model::ModelVertex()); // 미리 데이터의 크기를 잡아놓는 함수

			void* ptr = (void*)&(vertices[0]); // 시작 주소를 얻어온 다음
			r->Byte(&ptr, sizeof(Model::ModelVertex) * count); // 바이트로 읽어오는 함수


			mesh->vertices = new Model::ModelVertex[count]; // 모델에 할당
...
void Model::BindBone()
{
	...
	for (ModelBone* bone : bones)
	{
		if (bone->parentIndex > -1) // parentIndex가 -1이면 해당 bone은 root
		{
