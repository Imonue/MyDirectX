...
Transform::Transform() // SRT나 위치를 마킹하기 위해서 사용하는 생성자
...
void Transform::SetShader(Shader* shader)
{
	...
	sBuffer = shader->AsConstantBuffer("CB_World"); // 쉐이더에 월드를 넘겨주기 위해서 사용
}
...
{
	Math::MatrixDecompose(matrix, scale, rotation, position); // Matrix에서 SRT를 분해해오는 함수
